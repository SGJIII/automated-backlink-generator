...
def generate_outreach_email_content(website, user_id, sender_name, company_name, company_profile, article_url, author_name, is_followup=False):
    ...
    try:
        ...
        if len(parts) > 1 and parts[0].lower().startswith('subject:'):
            subject_line = parts[0][len('Subject:'):].strip()  # Remove 'Subject:' and strip whitespace
            body = parts[1].strip()
        else:
            subject_line = f"Collaboration opportunity from {company_name}"
            body = email_content
        ...
        return {
            'subject': f"Introduction to {company_name}",
            'body': generate_fallback_email(website, sender_name, company_name, company_profile, article_url, author_name, is_followup)
        }
...
def generate_pr_outreach_email_content(website, user, campaign, author_name):
    press_release_content = campaign.press_release_content
    prompt = f"""
You are an assistant that helps generate PR outreach emails.

Compose an email to {author_name}, an author at {website.url}, introducing {user.company} and sharing the following press release:

{press_release_content}

The email should be professional, personalized, and aim to get the author's attention to consider featuring or mentioning the press release in their future articles.

Ensure the email is friendly, concise, and follows proper email etiquette.

Do not mention that this email was generated by an AI.
"""
    try:
        response = anthropic.messages.create(
            model="claude-3-opus-20240229",
        ...
        parts = email_content.split('\n', 1)
        if len(parts) > 1 and parts[0].lower().startswith('subject:'):
            subject_line = parts[0][len('Subject:'):].strip()
            body = parts[1].strip()
        else:
            subject_line = f"Press Release from {user.company}"
            body = email_content
        return {
            'subject': subject_line,
            'body': body
        ...
        print(f"Error generating email with Anthropic API: {str(e)}")
        return {
            'subject': f"Press Release from {user.company}",
            'body': generate_fallback_email(website, user.name, user.company, user.company_profile, campaign.press_release_url, author_name)
        }
